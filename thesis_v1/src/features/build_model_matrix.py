...
# Primary financial target (real Bloomberg global A&D, USD — supervisor audit fix).
PRIMARY_TARGET = "r_WAERLST"
# Secondary target (real Bloomberg European defense, EUR).
SECONDARY_TARGET = "r_BSHIELDT"
# Tertiary target (European defense basket via yfinance).
TERTIARY_TARGET = "r_EUDEF"
# US comparison (kept for geographic robustness, no longer primary).
US_COMPARISON_TARGET = "r_ITA"
...
def build_targets(
    ...
    # Extra targets (tertiary + US comparison) for the expanded scope.
    if extra_targets:
        for t in extra_targets:
            if t and t in feature_matrix.columns:
                _add_target(t, t)
...
def build_info_sets(
    ...
    """Build the column lists for each information set.

    Nesting: F ⊂ P, F ⊂ N, P∪N ⊂ PN, PN ⊂ PNG.
    - F   : financial baseline
    - P   : F + physical attacks
    - N   : F + news attention (aggregate counts, tones, z-scores)
    - PN  : F + P + N + per-query × per-group news columns
    - PNG : PN + narrative-gap features
    """
    ...
    base_excludes = {
        "date",
        "ITA", "BSHIELDT", "WAERLST", "WAERLST_recon",
        "WAERLST_VOLUME", "BSHIELDT_VOLUME",
        # Target source columns are NOT excluded — their lagged versions
        # (r_WAERLST_lag1, r_ITA_lag1, etc.) are legitimate AR(1) features.
        # Only the raw (un-lagged) return sources are excluded above.
    ...
    # Force nesting: F ⊂ P, F ⊂ N, P∪N ⊂ PN, PN ⊂ PNG.
    # N must include F (financial + news) per config/model_config.yaml.
    # PN must include both P (attacks) and N (aggregate news) plus its
    # own per-query × per-group columns, so PN = F + attacks + all news.
