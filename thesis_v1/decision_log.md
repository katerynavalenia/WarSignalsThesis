> **⚠️ ARCHIVED — v1 only.** New decisions go in `docs/v2/decision_log.md`.
> This log is preserved unchanged as the historical record of v1.
...
## 2026-07-01 — ITA ETF as primary financial target (supersedes WAERLST)

**Decision:**
Use the iShares U.S. Aerospace & Defense ETF (ITA, fetched free via yfinance) as the **primary** financial target for all return and volatility forecasts. The Bloomberg WAERLST global aerospace & defense index is retained as a **secondary robustness** target (`r_WAERLST_recon`), reconstructed from Bloomberg constituent-level data.

**Reason:**
The Phase 1 financial audit found that the Bloomberg delivery contained only constituent-level close prices (no index-level series, no OHLC). The mcap-weighted reconstruction of WAERLST is too noisy (ρ=0.15 vs ITA, std 2.4× higher) due to small-cap and multi-currency constituents. ITA is a real, liquid, USD-denominated defense ETF with 1,613 days of full 6.5-year history. It covers the same defense universe as WAERLST (narrower geography but same sector). The decision log entry of 2026-06-28 ("WAERLST as main financial outcome") is superseded by this entry.

**Alternatives considered:**
- Keep WAERLST reconstruction as primary (rejected: too noisy; reconstruction artifacts dominate the forecast error).
- Use BSHIELDT as primary (rejected: also reconstructed; reserved for European robustness).
- Use a different free defense ETF (rejected: ITA is the most liquid and longest-history defense ETF available via yfinance).

**Consequences:**
- All primary return targets use `r_ITA`. All primary volatility targets use `target_var_r_ITA_t{1,5}`.
- `r_WAERLST_recon` is the secondary target for geographic robustness (European/global defense).
- The thesis must justify the ETF proxy in the Data chapter and acknowledge the geography limitation (ITA is U.S.-listed; WAERLST is global).
- The decision log entry of 2026-06-28 ("WAERLST as main financial outcome") is formally superseded.

**Revisit condition:**
If Bloomberg delivers the actual WAERLST index-level total-return series (not constituent-level), switch back to WAERLST as primary.

## 2026-07-01 — Fix N info set nesting (N was news-only, not F + news)

**Decision:**
Fix the `build_info_sets` function in `src/features/build_model_matrix.py` so that the N information set includes the F (financial) baseline columns, and PN includes both P (attacks) and N (aggregate news) columns. The previous code left N as a standalone news-only set (no financial features) and PN did not include N's aggregate news columns.

**Reason:**
The config `model_config.yaml` defines N as "financial + news" and PN as "financial + physical + news". But the nesting code only did `F ⊂ P ⊂ PN ⊂ PNG`, leaving N outside the chain. This meant:
- N had 21 news-only columns (no financial controls), making the N vs F comparison meaningless (news-only vs financial-only, not news+financial vs financial-only).
- PN had 74 columns (F + attacks + per-query news) but was missing the 26 aggregate news columns from N. The config says PN = "financial + physical + news" but the code excluded aggregate news.
- The Phase 7 audit (§1.5) noted "N=21=F=23 cardinality redundancy" — this was a symptom of the bug, not a design choice.

**Alternatives considered:**
- Drop the N info set entirely (rejected: the research plan §13 H2 requires testing news-only vs financial-only; N is needed for this).
- Keep N as news-only and document it as a "news-only" set (rejected: contradicts the config and the research plan).

**Consequences:**
- N now has 52 features (F's 26 + 26 aggregate news columns).
- PN now has 104 features (F + P + N + 16 per-query × per-group columns).
- PNG now has 107 features (PN + 3 narrative-gap columns).
- The model matrix was rebuilt and pushed to Drive. The Phase 7 Colab re-run will use the corrected info sets.
- The `ml_tuning.py` script was also fixed to reconstruct info sets from the model matrix columns if parquet attrs are missing (defensive fix matching `phase7_run_ml.py`'s `_load_model_matrix`).

**Revisit condition:**
If the N info set's 52 features cause XGBoost tuning to overfit (unlikely given max_depth=3-5), reduce N to a curated subset of the most important news features.
