...
| Power of that null | **Depends on the target, and the two must not be quoted as one.** On the three long-sample targets (1,855 OOS days): 0.2% → 40%, **0.5% → 81%**, 1.0% → 98%. On the two Bloomberg targets (686 OOS days): 0.2% → 21%, **0.5% → 44%**, 1.0% → 76%. Simulated at 1,000 paths per point, so SE ≤1.6pp. Power is not exclusion: 81% means one sample in five misses the effect | `outputs/tables/thesis_power_curve.csv` (supersedes `forecast_power_curve.csv`, which was the long sample only at 60 paths) |
...
| Physical air-attack intensity does not forecast defence-equity returns | 144 specifications across F/P/N/PN/PNG, 3 targets, 2 horizons, 2 samples; 16 with positive R²_OS, **0 surviving BH**. Baselines win 8 of 12 cells. **Not pre-registered — exploratory** | `outputs/tables/horse_race.csv` |
| Nor do war signals forecast defence-equity **volatility** | 48 augmented HAR-RV-X specifications against GARCH/GJR/EGARCH benchmarks under QLIKE: **0 significantly better**, 8 significantly worse, and all 6 BH survivors are deteriorations. **Not pre-registered — exploratory** | `outputs/tables/volatility_race.csv` |
| War-variable MAE gains exist but are indistinguishable from noise | A war set beats every baseline on MAE in 5 of 12 cells, by 0.17–2.98%; none survives correction. The approved thesis reports a comparable 0.4% ITA exception | `outputs/tables/horse_race_baseline_vs_extended.csv` |
| v1's weekend grid inflates its own out-of-sample fit | Its targets repeat across Sat/Sun (388 of 1,358 rows). On v1's own matrix, dropping weekend rows lowers R²_OS in all 16 cells, by 0.038 on average — but the winning information set is unchanged in every one. Unaffected by our later alignment fix, since it runs on v1's targets | `outputs/tables/v1_weekend_diagnostic.csv` |
