...
**Phase 7 — Machine-learning models** 🟡 **Headline run complete; h=5 + GARCH-X pending** (2026-07-01)
...
- **Headline run (h=1 only, no GARCH-X)**: ✅ Complete. 50 return benchmark rows, 10 tuned hyperparams, 190 SHAP arrays, 10 SHAP figures on Drive and pulled to local. See [`docs/phase7_audit.md`](phase7_audit.md).
- **Headline result**: H1/H2/H3 null on returns (h=1). XGBoost matches HistoricalMean on F, marginally improves on PN (0.28% better MAE, 0.552 vs 0.540 dir_acc). XGBoost's native NaN handling makes it immune to the train/test distribution shift that penalizes OLS/Ridge on P/PN/PNG.
- **Pending (C7/C8 follow-up)**: h=5 horizon + GARCH-X variants. Re-run with `--horizons 1,5 --garch-x-info-set F` to close.
