...
The descriptive section uses a small number of tables and figures so that the thesis stays within the professor's recommended limit of ten total tables and figures. The selected outputs show the financial targets, the physical and narrative war signals, the main variable definitions, descriptive statistics, the validation of the source classification, forecast performance, and robustness checks. This keeps the empirical presentation compact and focused on the research question.
...
\begin{table}[H]
\centering
\caption{Descriptive statistics for selected modeling variables}
...
\begin{tablenotes}
\small
\item Notes: WAERLST and BSHIELDT return statistics are computed from the official Bloomberg index files over the model-window period beginning 29 September 2022. Attack statistics are over the 809 dates carrying a Ukrainian Air Force record. News statistics are over the full 4,027-day extraction; article counts are medians because the distributions are strongly right-skewed, and tone is the mean GDELT tone of conflict-related articles. Returns are in percent.
\end{tablenotes}
\end{threeparttable}
\end{table}
...
The second is the precision of the outlet register. Because the classifier is a deterministic function of the publishing domain, the precision of its article assignments is the precision of its domain assignments weighted by article volume. The register is therefore checked domain by domain against Wikidata, an independently maintained source, matching each outlet by its official website rather than by name. Of the 84 registered outlets, 66 can be verified in this way and 63 of those agree, a precision of 0.955. The Ukrainian, Russian state, and Western groups return no disagreement at all. All three disagreements are exile newsrooms, for which Wikidata records the country of legal registration while the register records the country whose readership the outlet serves; these are recorded and not acted upon.

The third is sensitivity to the classification rule itself. The central comparison is re-estimated under four alternatives: using only the curated register, dropping the language tier, restoring the excluded syndication platforms, and resolving language before country. Under the primary alignment the number of specifications surviving correction is one under the rule used here and between one and two under every alternative, so the answer does not depend on these choices. The language-first rule is instructive in a second way: it cannot produce a Russian independent group at all, because Russian-language articles from independent outlets are claimed by the language tier before the register is consulted. That is the concrete cost of resolving language before country.
...
\toprule
Model & Information set & Horizon & MAE & RMSE & Directional accuracy & $R^2_{OS}$ \\
\midrule
...
XGBoost & N & 5 & 2.807 & 3.424 & 0.448 & -0.178 \\
...
\begin{tablenotes}
\small
\item Notes: The table reports WAERLST out-of-sample forecasts on the measured sample. MAE and RMSE are in return percentage points. Directional accuracy is the share of dates on which the predicted and realized return have the same sign. $R^2_{OS}$ is the Campbell--Thompson out-of-sample $R^2$ against the historical-mean benchmark. Both horizons use 145 forecast dates. F is the financial baseline, P adds physical attack variables, N adds news variables, PN combines physical and news variables, and PNG adds narrative gaps. No specification in this table survives false-discovery-rate correction across the full grid.
\end{tablenotes}
\end{threeparttable}
\end{table}
...
\begin{figure}[H]
\centering
\includegraphics[width=1\textwidth]{fig11_target_distribution.png}
\caption{Distribution of the one-day WAERLST return forecasting target. The left panel reports daily log returns in percent. The right panel reports the same distribution with a logarithmic count axis, which makes the tails of the return distribution easier to see.}
\label{fig:appendix_target_distribution}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=1\textwidth]{fig10_master_coverage.png}
\caption{Monthly data coverage by source group. Each line reports the share of non-missing observations for the corresponding source group in a given month. The dashed vertical line marks the beginning of the common modeling window on 29 September 2022.}
\label{fig:appendix_coverage}
\end{figure}
