...
\textbf{The second feature pre-empts the most obvious objection to the results that follow.} Panel D reports the daily changes that are the actual regressors, and shows that the local series are not inert. Those changes are mean-zero by construction, so it is their dispersion that matters: the daily change in Ukrainian attention has a standard deviation of 2.3 percentage points and that in Russian independent attention 4.1, against 1.2 for Western outlets and 1.0 for native-English ones. The local regressors therefore vary two to four times as much from day to day as the Western ones. Whatever explains the null in Section~\ref{sec:findings}, it is not an absence of variation in the local regressors.
