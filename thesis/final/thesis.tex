...
The primary financial targets are two official Bloomberg defence-equity indices: the Bloomberg World Large, Mid and Small Aerospace \& Defense Total Return Index (\textbf{WAERLST}, USD) and the Bloomberg Europe Defense Select Total Return Index (\textbf{BSHIELDT}, EUR), both running from 2 January 2020 to 29 June 2026 (1,619 trading days, no missing observations). Two robustness targets extend coverage backwards and separate the two regions: the iShares U.S.\ Aerospace \& Defense ETF (\textbf{ITA}), which is available for the whole 2015--2026 window and so covers the pre-war baseline the Bloomberg indices miss, and a hand-built equal-weighted basket of European defence names (\textbf{EU basket}).
...
\item Two abbreviations recur in the results tables. \textbf{BH survivors} is the number of specifications that remain significant after Benjamini--Hochberg correction \citep{benjamini1995} controlling the false discovery rate at 5\% across all cells of a grid simultaneously; it is the count that matters, not the number of nominally significant cells. \textbf{HAC(5)} denotes Newey--West standard errors \citep{newey1987} with a five-day bandwidth, which allow for serial correlation and heteroskedasticity in daily returns.
\end{tablenotes}
\end{threeparttable}
\end{table}
...
\begin{tablenotes}
\small
\item Notes: Produced by \texttt{scripts/make\_thesis\_descriptives.py}. Panels A--D cover each series over the full span on which it exists; the two Bloomberg indices begin on 2 January 2020, the news series on 18 February 2015, and the news series end on 20 May 2026. Panel E covers the 809 market-information dates carrying at least one recorded weapon launch, between 29 September 2022 and 21 June 2026.
\item \textbf{Panel B is the single most important descriptive fact in the data.} Ukrainian, Russian state and Russian independent outlets devote 71--85\% of their output to this conflict on an average day, against 5--7\% for Western and native-English outlets --- an order-of-magnitude difference in editorial attention that persists across the whole eleven years. The local ecosystems are also far less variable around those means (Ukrainian s.d.\ 5.0 percentage points on a mean of 81.0), because they were already close to their ceiling before the invasion and had little room to rise further.
\item \textbf{Panel D is included because the regressions use changes, not levels}, and a reader should be able to see the dispersion of the variable actually entering equation~(\ref{eq:main}). The daily changes are mean-zero by construction and, unlike the levels, are not dominated by the war-regime trend. Note that the local attention series are two to four times more volatile day to day than the Western ones, so the null reported in Section~\ref{sec:findings} is not a case of a local regressor that fails to vary.
\end{tablenotes}
\end{threeparttable}
\end{table}
...
\item[$b$] The two Gate 3 primary survivors are both weekly and both in the 2025--26 episode window (58 observations against 13 parameters). The two deepest cells in the grid, the pooled daily specifications with 2,754 observations each, return $p = 0.08$ and $p = 0.36$. Neither pre-registered arm clears: Arm 1 requires a survivor in the Russia window on a Bloomberg target, and the only Russia-window survivor uses the hand-built basket that the pre-registration explicitly excluded; Arm 2 requires same-sign survivors in two independent windows, and the Russia daily cell carries the opposite sign to all six weekly cells.
\end{tablenotes}
\end{threeparttable}
\end{table}
...
\textbf{Gate 4 is also this project's clearest illustration of how a positive can appear and then dissolve under replication.} The exploratory version of this test, run on the 81 days of build-up coverage that had been ingested at the time, returned $p = 0.0005$. The pre-registered confirmatory run --- same asset, same specification, same controls, same calendar period, and nothing changed but the quantity of data --- returned $p = 0.399$ on 222 continuous days. The result was not overturned by a better control or a stricter correction. It was overturned by more of the data it had always been about, which is the failure mode that pre-registration exists to expose.
