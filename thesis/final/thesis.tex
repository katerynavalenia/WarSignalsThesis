...
\begin{tablenotes}
\small
\item Notes: Dependent variable is the daily percentage return on BSHIELDT, the Bloomberg Europe Defense Select Total Return Index, over the 921 days on which it trades and all five news series exist. HAC(5) standard errors in parentheses. $^{*}$, $^{**}$ and $^{***}$ denote significance at 10\%, 5\% and 1\%. All news regressors are daily first differences, standardised to unit variance, so a coefficient of $-0.125$ means that a one-standard-deviation rise in the change in Russian independent attention accompanies a 0.125 percentage-point lower defence return that day. Produced by \texttt{scripts/make\_thesis\_regression.py}, which reuses the specification of the gate tests exactly.
\item Columns (3) and (4) differ in one respect only --- whether coverage is credited to the day it was published or to the next trading day --- and that single change is what moves the local block from jointly significant at $p = 0.003$ to indistinguishable from zero at $p = 0.734$. The individual coefficients move with it: Russian independent attention goes from $-0.125$ to $0.010$, and Russian state tone from $0.127$ to $-0.055$. Note also how little any of this matters for fit. The market control alone explains 34.8\% of the variation in daily European defence returns; adding all ten news series raises that to 36.5\% at the untradeable alignment and 35.4\% at the tradeable one.
\end{tablenotes}
\end{threeparttable}
\end{table}
...
The table below supports the descriptive section and is placed here rather
than in the body to keep the number of exhibits in the paper itself to ten.
